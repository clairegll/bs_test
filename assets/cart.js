...
// Exercice Moyen en JS
// Inspiration https://nozzlegear.com/shopify/using-javascript-to-manage-a-shopify-cart & ChatGPT
...
async function getCartContents() {
  try {
      ...
      if (result.status === 200) {
          ...
          console.error(`Failed to fetch cart data, Shopify returned ${result.status} ${result.statusText}`);
          return null;
      }
  } catch (error) {
...
async function updateCartItemQuantity(variantId, quantity) {
  try {
      ...
      if (result.status === 200) {
          ...
          console.error(`Failed to add item to cart, Shopify returned ${result.status} ${result.statusText}`);
          return null;
      }
  } catch (error) {
      console.error("An error occurred:", error);
...
updateCartItemQuantity(variantIdToUpdate, newQuantity);
...
async function addItemToCart(variantId, quantity, properties = {}) {
  try {
    ...
    if (result.status === 200) {
      const addedLineItem = await result.json();
      console.log("Added Line Item:", addedLineItem);
...
addItemToCart(variantIdToAdd, quantityToAdd);